...
export class RequestCreateTransactionModel extends Object {
  ...
  actualAmount() {
    if (this.amount != undefined) {
      var discountAmount =
        this.dataDetail?.dataView?.discountAmount?.length != 0 ? this.dataDetail?.dataView?.discountAmount : '0';
  ...
  setAdditionalFields(gatewayCode: string) {
    ...
    this.description =
      this.customerName + ' ' + this.serviceName?.toLowerCase() + ' ' + this.hospitalName?.toLowerCase();
